...
const WeeklyHoroscope = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (entryId) {
        ...
        alert("Weekly horoscope updated!");
      } else {
        await createHoroscope(payload);
        alert("Weekly horoscope created!");
      }
      loadHoroscope();
    } catch (err) {
      alert("Error saving weekly horoscope");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    const confirmDelete = window.confirm(
      "Are you sure you want to delete this weekly horoscope?"
    );
    if (!confirmDelete) return;

    try {
      await deleteHoroscope(entryId);
      alert("Weekly horoscope deleted!");
      ...
      alert("Error deleting weekly horoscope");
    }
  };
    ...
    <div className="max-w-4xl mx-auto bg-white p-6 rounded-xl shadow-lg border border-gray-200">
      ...
      <div className="space-y-4">
        <div>
          ...
          <input
            type="text"
            placeholder="Enter weekly horoscope title"
            value={title}
        ...
        <div>
          ...
          <textarea
            rows="6"
            placeholder="Enter weekly horoscope description"
            value={content}
        ...
        >
          {entryId ? "Update Weekly Horoscope" : "Create Weekly Horoscope"}
        </button>
