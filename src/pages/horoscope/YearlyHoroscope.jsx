...
const YearlyHoroscope = () => {
  ...
  // Load horoscope when sign or year changes
  ...
  const handleSave = async () => {
    ...
    try {
      if (entryId) {
        ...
        alert("Yearly horoscope updated!");
      } else {
        await createHoroscope(payload);
        alert("Yearly horoscope created!");
      }
      loadHoroscope();
    } catch (err) {
      alert("Error saving yearly horoscope");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    const confirmDelete = window.confirm(
      "Are you sure you want to delete this yearly horoscope?"
    );
    if (!confirmDelete) return;

    try {
      await deleteHoroscope(entryId);
      alert("Yearly horoscope deleted!");
      ...
      alert("Error deleting yearly horoscope");
    }
  };
    ...
    <div className="max-w-4xl mx-auto bg-white p-6 rounded-xl shadow-lg border border-gray-200">
      ...
      <div className="space-y-4">
        <div>
          ...
          <input
            type="text"
            placeholder="Enter yearly horoscope title"
            value={title}
        ...
        <div>
          ...
          <textarea
            rows="6"
            placeholder="Enter yearly horoscope description"
            value={content}
        ...
        >
          {entryId ? "Update Yearly Horoscope" : "Create Yearly Horoscope"}
        </button>
